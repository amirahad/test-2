...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/agents", async (req, res) => {
    try {
      ...
      const rawData = insertAgentSchema.parse(req.body);
      
      // Remove agencyId to avoid the column error
      // Create a new object without agencyId
      const { agencyId, ...dataWithoutAgencyId } = rawData;
      
      // Create the agent with the sanitized data
      const agent = await storage.createAgent(dataWithoutAgencyId);
  ...
  app.patch("/api/agents/:id", async (req, res) => {
    try {
      ...
      // Parse data with the schema
      const rawData = insertAgentSchema.partial().parse(req.body);
      
      // Remove agencyId to avoid the column error
      const { agencyId, ...dataWithoutAgencyId } = rawData;
      
      const updatedAgent = await storage.updateAgent(id, dataWithoutAgencyId);
  ...
  app.post("/api/transactions", async (req, res) => {
    try {
      // Parse data with the schema
      const rawData = insertTransactionSchema.parse(req.body);
      
      // Remove agencyId to avoid the column error
      // Create a new object without agencyId
      const { agencyId, ...dataWithoutAgencyId } = rawData;
      
      const transaction = await storage.createTransaction(dataWithoutAgencyId);
  ...
  app.patch("/api/transactions/:id", async (req, res) => {
    try {
      ...
      // Parse data with the schema
      const rawData = insertTransactionSchema.partial().parse(req.body);
      
      // Remove agencyId to avoid the column error
      const { agencyId, ...dataWithoutAgencyId } = rawData;
      
      const updatedTransaction = await storage.updateTransaction(id, dataWithoutAgencyId);
  ...
  app.get('/api/export/transactions/csv', async (req, res) => {
    try {
      ...
      const rows = transactions.map(t => {
        return `${t.propertyAddress || ''},${t.propertySuburb || ''},"",${t.propertyType || ''},${t.price || ''},${t.status || ''},${t.listedDate ? new Date(t.listedDate).toISOString().split('T')[0] : ''},${t.transactionDate ? new Date(t.transactionDate).toISOString().split('T')[0] : ''},${t.agentName || ''},"",${t.bedrooms || ''},${t.bathrooms || ''},"","",""`;
      }
      ).join('\n');
